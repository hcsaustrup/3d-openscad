module __rotaryencoder(thickness, type, ht=6, positive) {

    rotary_bottom_width = 20;
    rotary_bottom_height = 15;

    rotary_box_width = 12.2;
    rotary_box_height = 12.2;
    rotary_box_depth = 1;
    rotary_box_thickness = 1;

    rotary_thread_diameter = 7.0;
    rotary_thread_height = ht;
    ...
    module __build(thickness) {
        ...
        difference() {
            union() {
                translate([
                    -(rotary_box_width/2)-(rotary_box_thickness),
                    -(rotary_box_height/2)-(rotary_box_thickness)
                ])
                    cube([
                        rotary_box_width+(2*rotary_box_thickness),
                        rotary_box_height+(2*rotary_box_thickness),
                        rotary_thread_height + rbd
                    ]);
            }
            union() {
                cylinder(h=rotary_thread_height, r=rotary_thread_diameter/2);

                // cylinder(h=rotary_nut_height, r=rotary_nut_diameter/2);

                translate([0,0, rotary_thread_height-rotary_ridge_height])
                    cylinder(h=rotary_ridge_height, r=rotary_ridge_diameter/2);

                translate([
                    -(rotary_box_width/2),
                    -(rotary_box_height/2),
                    rotary_thread_height
                ])
                    cube([
                        rotary_box_width,
                        rotary_box_height,
...
module rotaryencoder_boxed(thickness, ht, positive) {
    __rotaryencoder(thickness, "boxed", ht, positive);
}

module rotaryencoder_nudge(thickness, ht, positive) {
    __rotaryencoder(thickness, "nudge", ht, positive);
}
